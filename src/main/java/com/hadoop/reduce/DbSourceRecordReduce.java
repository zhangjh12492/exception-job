 ...
 *
 */
public class DbSourceRecordReduce extends MapReduceBase implements Reducer<LongWritable, Text, MesAllProcessReq, Text> {
	...
	public void reduce(LongWritable key, Iterator<Text> val, OutputCollector<MesAllProcessReq, Text> output, Reporter reporter) throws IOException {
		...
		while (val.hasNext()) {
			...
			mesAllProcessReq = objectMapper.readValue(value, MesAllProcessReq.class);
